...
import { DISCOVER_INTRO, VISITING_PROGRAM_ENTRY, VISITING_PROGRAM_INTRO, VISITING_PROGRAM_PIC_URL } from '_data/config';
...
export default function VisitingProgram() {
  return (
    <RootStyle>
      <Container>
        <Grid container spacing={3} justifyContent="space-between" alignItems="center">
          ...
          >
            <Image
              alt="teams"
              src={VISITING_PROGRAM_PIC_URL}
            />
          </Grid>

          <Grid item xs={12} md={6} lg={6} sx={{ textAlign: { xs: 'center', md: 'left' } }}>
            <Typography variant="h2">Visiting Program</Typography>
            <Typography sx={{ mt: 3, mb: 5, color: 'text.secondary' }}>
              {VISITING_PROGRAM_INTRO}
            </Typography>
            ...
            {VISITING_PROGRAM_ENTRY.map((item) => (
              <Button
                id={item.id}
                variant="outlined"
                color="inherit"
